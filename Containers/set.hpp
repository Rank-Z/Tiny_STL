...
public:
	...
	{
		return _tree.insert_unique(_STD move(value));
	}
	...
	_STD pair<iterator , bool> emplace(Args&&...args)
	{
		...
		_STD pair<iterator , bool> ret(_tree.insert_unique(_STD move(*vp)));
		if (ret.second == false)
	...
	iterator emplace_hint(const_iterator hint , Args&&...args)
	{
		...
		return (_tree.insert_unique(_STD move(*vp))).first;
	}

	iterator erase(iterator pos)//since C++17 return iterator ,void before
	{
	...
	{
		nodeptr np = _tree.find(key);
		if (np == nullptr)
			return cend();
		else
			return _tree.make_iter(np);
	}
